    ...
    return (
        <section id="projects" className="bg-[var(--bg-shade-1)] py-16 px-[85.333px]">
            ...
            {projects.map((project, index) => (
                <div onClick={() => navigate(`/${project.id}`)} key={index} className="h-[360px] bg-white p-4 rounded-2xl transition-all transform shadow-[0px_4px_8px_rgba(0,0,0,0.05)] hover:shadow-[0px_6px_12px_rgba(0,0,0,0.15)] hover:-translate-y-2 cursor-pointer">
                    <img
                        src={project.image}
                        alt={project.title}
                    ...
                    <p className="text-sm text-[var(--text-color)] mb-2">{project.shortDescription}</p >
